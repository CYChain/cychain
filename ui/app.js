...
/**
 * Main entry point.
 * Loads the cychain modules, the cychain api and run the express server as Domain master.
...
/**
 * The config object to handle cychain modules and cychain api.
	...
	async.auto(
			...
			listen: [
				...
				function(scope, cb) {
					scope.network.server.listen(
						...
						err => {
							scope.logger.info(
								`cychain started: ${scope.config.address}:${scope.config.httpPort}`
							);
		...
		(err, scope) => {
			if (err) {
				 ...
				 * @property {Object} ed - Crypto functions from cychain node-sodium.
